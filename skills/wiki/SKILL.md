...
| "high fidelity ingest", "quality-first ingest", "re-ingest with more detail" | HQ INGEST | `wiki-ingest-hq` |
| "force ingest and repair", "ingest then lint and fix", "one-command HQ ingest" | HQ AUTO INGEST | `wiki-ingest-hq-auto` |
...
## Project-Specific HQ Extensions

This repository includes two local extensions on top of the upstream `claude-obsidian` flow:

- `wiki-ingest-hq`: use when ingest quality and query-readiness matter more than speed
- `wiki-ingest-hq-auto`: use when the user wants ingest, focused lint, repair, and re-check chained together

When the user explicitly emphasizes fidelity, detail, stronger cross-linking, or one-command repair after ingest, prefer these extensions over the base `wiki-ingest` flow.
